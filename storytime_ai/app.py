...
from storytime_ai import Dialog, Story
...
css = Path(str(files("storytime_ai.templates").joinpath("webapp.css"))).read_text()
hide_label = f"<style>\n{css}\n</style>"
...
@st.cache_data
def load_stories():
    storyfiles = getfilelist(Path(str(files("storytime_ai.templates").joinpath(""))), "md", withpath=True)
    stories = [
...
async def aync_next_dialog(nextdialog: Optional[str] = None, t=None):
...
def next_dialog(nextdialog: Optional[str] = None, t=None):
...
with st.sidebar.expander("Select template stories"):
    ...
    #                 <div style='display:inline-block; text-align:right; width:20%; font-size:0.7em'>
    #                   {r.number_of_dialogs} dialogs</div>
...
def create_story():
    dialog = Dialog.from_markdown(f"## {st.session_state.newstorydialogtitle}\n{st.session_state.newstorydescription}")
...
if new_story:
    with st.form("newstory"):
        st.text_input(
            "New story title",
            value=st.session_state.story.title if st.session_state.story else "Story",
            key="newstorytitle",
        )
        st.text_input(
            "First dialogue title",
            value=st.session_state.story.currentdialog.dialogid if st.session_state.story else "",
            key="newstorydialogtitle",
        )
        st.text_area(
            "First dialogue description",
            value=(st.session_state.story.currentdialog.text if st.session_state.story else "")
            + (st.session_state.story.currentdialog.choices_to_markdown() if st.session_state.story else ""),
            key="newstorydescription",
        )
        st.form_submit_button("Create new story", on_click=create_story)
else:
    st.markdown("## " + (st.session_state.story.title if st.session_state.story else ""))
    t = st.empty()
    t.markdown(
    ...
    for choice, choiceobject in (
        st.session_state.story.currentdialog.choices.items() if st.session_state.story else {}.items()
    ):
        st.button(
